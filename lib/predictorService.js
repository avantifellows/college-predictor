...
export function percentileToAIR(percentile, totalCandidates = TOTAL_CANDIDATES) {
...
export async function predictFromMarks(marksOutOf300, category, noiseStd = 0.0) {
    try {
        if (!session) {
            await loadModel();
        }
    ...
    try {
        ...
        for (const student of studentsData) {
            ...
            if (typeof marks !== 'number' || marks < 0 || marks > 300) {
                throw new Error(`Invalid marks for student ${name || 'unknown'}: ${marks}`);
            }

            const validCategories = ['GEN', 'OBC', 'SC'];
            if (!validCategories.includes(category.toUpperCase())) {
                throw new Error(`Invalid category for student ${name || 'unknown'}: ${category}`);
            }
