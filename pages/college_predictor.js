...
const CollegePredictor = () => {
  ...
  const applyQuotaFilter = (data) => {
    ...
    // For all exams except JEE Main-JOSAA, return all data without filtering
    if (exam !== "JEE Main-JOSAA") {
      return data;
    }
    
    // Only apply quota filtering for JEE Main-JOSAA
    const filtered = data.filter((item) => {
      if (selectedQuota === "AI") {
        return true; // Show all colleges for All India
      ...
      return true; // Default case - show all colleges
    });
