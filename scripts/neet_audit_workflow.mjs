export const meta = {
  ...
  description: 'Audit each NEET state cutoff source for parsing/data bugs against sanity rules + web-verified anchors',
  phases: [
    { title: 'Audit', detail: 'one agent per state source: scan anomalies, read parser, web-verify anchors' },
    { title: 'Verify', detail: 'adversarially confirm each reported bug is real (not a real-world quirk)' },
    { title: 'Synthesize', detail: 'merge into one ranked bug report with root cause + fix per source' },
  ],
}

const REPO = '/Users/surya/jan2023/college-predictor'
const PARSERS = '/Users/surya/jan2023/futures-v2/neet/scrape/scripts'
const SCAN = '/private/tmp/claude-501/-Users-surya-jan2023-college-predictor/3491cd03-fb57-4cf5-a1e1-ea8bbe09cd7a/scratchpad/anomaly_scan.py'

// Each state: source substring in NEETUG.json + its parser file + notable colleges to web-verify.
const STATES = [
  { key: 'maharashtra', src: 'maharashtra', parser: '06_parse_maharashtra.py',
    anchors: 'Grant Medical College Mumbai (GSMC), BJ Medical College Pune, IIMSR Jalna minority',
    focus: 'quota-vs-category (I.Q. is a QUOTA not a category); EM = EarMarking (EMR=Receiver / EMD=Donor), NOT "EWS Minority" — the parser mislabels it; giant closing ranks (>700000) at GMC Jalna suggest wrong-college attribution in the regex; minority male worse than female at IIMSR Jalna.' },
  { key: 'telangana', src: 'telangana', parser: '07_parse_telangana.py',
    anchors: 'Osmania Medical College Hyderabad, Gandhi Medical College Secunderabad, Kakatiya MC Warangal',
    focus: 'MOP-UP phase looser cutoffs; COLL:: text blocks; female split; category vs local/non-local quota.' },
  { key: 'kerala', src: 'kerala', parser: '09_parse_kerala.py',
    anchors: 'Govt Medical College Thiruvananthapuram, Kozhikode GMC',
    focus: 'Kerala state-rank converted to AIR via crosswalk; SM/SM-MU category codes; unmapped rows.' },
  { key: 'karnataka', src: 'karnataka', parser: '03_parse_karnataka.py',
    anchors: 'Bangalore Medical College BMCRI, Mysore Medical College',
    focus: 'KEA course code; GM/2AG/etc ~40 category codes; MBBS-GOVT/PRIV/NRI in course name; seat-type mapping.' },
  { key: 'gujarat', src: 'gujarat', parser: '02_parse_gujarat.py',
    anchors: 'BJ Medical College Ahmedabad, AMC MET, Government MC Surat',
    focus: 'pre-computed wide grid, category-as-columns OPEN/SC/ST/SE/EW; quota suffix GQ/MQ/NQ/LQ; Score+AIR carried; duplicate buckets (46 found).' },
  { key: 'westbengal', src: 'westbengal', parser: '04_parse_flat_states.py',
    anchors: 'Medical College Kolkata, RG Kar Medical College, NRS Medical College',
    focus: 'R1 only; UR/state categories; 16 duplicate buckets.' },
  { key: 'mp', src: 'mp_2025', parser: '04_parse_flat_states.py',
    anchors: 'Gandhi Medical College Bhopal, MGM Medical College Indore',
    focus: 'has AI RANK + MP STATE RANK + AISCORE; compound category UR/X/OP; ensure AI RANK (not state rank) is used.' },
  { key: 'punjab', src: 'punjab', parser: '04_parse_flat_states.py',
    anchors: 'Government Medical College Patiala, GMC Amritsar',
    focus: 'NEET Marks+Rank; 49 duplicate buckets (highest dup count among states) — investigate dedup/pivot key.' },
  { key: 'andhra', src: 'andhra', parser: '05_parse_andhra.py',
    anchors: 'Andhra Medical College Visakhapatnam, Guntur Medical College',
    focus: 'college name is a SECTION HEADER row (carry-forward); NEET RANK+Score; compound allotment string; carry-forward bugs (wrong college for a block).' },
  { key: 'himachal', src: 'himachal', parser: '08_parse_himachal.py',
    anchors: 'IGMC Shimla, Dr RPGMC Tanda',
    focus: 'cleanest table-based R3; only 34 rows; 6 duplicate buckets; MBBS/BDS program from name heuristic.' },
]
...
const FINDINGS_SCHEMA = {
  ...
  properties: {
    source: { type: 'string' },
    anomaly_counts: { type: 'string', description: 'summary of anomaly_scan output (rule: count)' },
    findings: {
      type: 'array',
      items: {
        type: 'object',
        properties: {
          ...
          category: { type: 'string', description: 'e.g. wrong-column, dup-bucket, quota-vs-category, contaminated-max, missing-college, mangled-field, real-world-quirk' },
          evidence: { type: 'string', description: 'concrete rows/values from the data + what reality says (with web source if applicable)' },
          root_cause: { type: 'string', description: 'the specific parser line/logic causing it, file:line if known' },
          proposed_fix: { type: 'string' },
          is_real_bug: { type: 'boolean', description: 'false if this is actually a legitimate real-world quirk, not a bug' },
        },
        required: ['title', 'severity', 'category', 'evidence', 'root_cause', 'is_real_bug'],
      },
    },
  },
...
const results = await pipeline(
  STATES,
  (s) => agent(auditPrompt(s), { label: `audit:${s.key}`, phase: 'Audit', schema: FINDINGS_SCHEMA, agentType: 'general-purpose' }),
...
Check the actual data/parser if needed (data at ${REPO}/public/data/NEETUG/NEETUG.json, parser at ${PARSERS}/${s.parser}). Verdict CONFIRMED only if it is genuinely a parser/data bug.`,
          { label: `verify:${s.key}:${f.title.slice(0, 24)}`, phase: 'Verify', schema: VERDICT_SCHEMA, agentType: 'general-purpose' }
        ).then((v) => ({ finding: f, ...v }))
      )
...
const packed = results.filter(Boolean).map((r) => ({
  ...
  confirmed: (r.verified || []).filter((v) => v.verdict === 'CONFIRMED').map((v) => v.finding),
  refuted: (r.verified || []).filter((v) => v.verdict !== 'CONFIRMED').map((v) => ({ t: v.finding.title, verdict: v.verdict, why: v.reasoning })),
}))
