#!/usr/bin/env python3
"""
Uttarakhand state min-marks matrix. Same L1->L5 machinery.

Decisions (docs/NEET_2026_MATRIX_DECISIONS.md, "STATE SLICE — UTTARAKHAND"):
- SOURCE = THEIRS (`UK_closing_ranks_state_govt_2025.csv`). Dropbox-only. PARSER AUDITED
  (state_UK.py): AIR-native (uses `neet_air` col; verified Doon Medical UR 23,789 = plausible top-UK
  AIR), keyword govt filter -> 5 govt MBBS colleges (Doon, Haldwani, Haridwar, VCSG Srinagar, SSJ
  Almora), STATE-QUOTA-only, gender decode. Extracted == raw (0 mismatch). Round = R3 broadsheet.
  Cross-check: UK AIQ UR 526 (neetugguidance) ≈ our top-college Doon 530; state floor 508 looser (ok).
- CATEGORIES: UR->Gen, EWS->Gen-EWS, OBC->OBC, SC->SC, ST->ST. Take looser-of-gender (max M/F).
- CAVEAT — THIN STATE: only 5 govt MBBS colleges; reserved pools very sparse (ST = 1 seat at AIR
  1.09M -> effectively qualifying; SC loosest-5 380k-800k; EWS deep). LOW CONFIDENCE for reserved
  categories. Parser docstring also warns R3-broadsheet may under-capture R1/R2-retained candidates.
- FLOOR = median of loosest-5 govt colleges (=~all 5). SEAT_GROWTH=1.008.
"""
...
THEIR = REPO / "amogh-csv/medical-state-counselling/extracted_data/UK_closing_ranks_state_govt_2025.csv"
...
def build_floors(prog):
    ...
    for r in csv.DictReader(open(THEIR)):
        ...
        if air > col[v][r["allotted_college"]]: col[v][r["allotted_college"]] = air
...
print("\nMBBS floors per vert:", {v: mbbs.get(v, 0) for v in ["UR", "EWS", "OBC", "SC", "ST"]})
print("CAVEAT: thin state (5 govt colleges); ST=1 seat (qual-gated); SC/EWS pools very sparse -> low confidence reserved.")

outp = OUT / "uk_matrix_final.csv"
...
print(f"\nUK: THEIRS (audited) AIR-native | looser-of-gender | median-of-5 (5 colleges, thin) | wrote {outp}")
