#!/usr/bin/env python3
"""
Madhya Pradesh state min-marks matrix. Same L1->L5 machinery.

Decisions (docs/NEET_2026_MATRIX_DECISIONS.md, "STATE SLICE — MADHYA PRADESH"):
- SOURCE = OURS (`mp_2025_cutoffs`, Round 1, AIR-native). Reconciled vs theirs on govt medical
  colleges (MGM Indore ours 8,510 vs theirs 11,467 ~1.35x; Bhopal/Gwalior/Jabalpur/Vidisha ~1.0-1.2x).
  STRONGEST model validation yet: their independent `closing_neet` marks match OUR AIR->marks curve
  to WITHIN 1 MARK across 349-516 (44158->506 vs 507, 317384->349 vs 349, etc.).
- GOVT FILTER = our `Seat Type == "GOVT"` — VERIFIED CLEAN for MP (17 real govt colleges; private
  Sri Aurobindo/LNCT correctly tagged Private). Unlike KA/PB, MP's govt flag is trustworthy, use as-is.
- CATEGORIES: compound code VERT/HORIZ/SUB. VERT ∈ {UR,OBC,SC,ST,EWS}; HORIZ ∈ {X(none/general),
  GS,PH,SN,FF}; SUB=OP(open-merit). Base floor uses the **/X/** general-horizontal variant:
  UR/X->Gen, EWS/X->Gen-EWS, OBC/X->OBC, SC/X->SC, ST/X->ST. PwD (PH horiz) excluded from base; PwD
  row = qualifying. R1 data -> runs strict (like AIQ).
...
# matrix row -> MP compound code (general /X/OP variant)
CODE_FOR = {"Gen": "UR/X/OP", "Gen-EWS": "EWS/X/OP", "OBC": "OBC/X/OP", "SC": "SC/X/OP", "ST": "ST/X/OP"}
...
def build_floors(prog):
    ...
    for r in d:
        ...
        if r["Seat Type"] != "GOVT": continue            # MP govt flag is clean (verified)
        c = r["Category"]
        if c in CODE_FOR.values():
...
print(f"\nMP: ours(AIR,R1) GOVT-flag(clean) | UR/X/OP etc general-horiz variant | median-of-5 | wrote {outp}")
